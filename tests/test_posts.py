...
def test_get_all_posts(authorized_client, test_posts):
    ...
    assert res.status_code == 305  # changed status code for testing
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("favorite pizza", "Dominoes lOVe", True),
        ("WWE", "wrestling sport", True),
    ],
)
...
def test_create_post_default_published(authorized_client, test_user):
    res = authorized_client.post(
        "/posts/", json={"title": "testing", "content": "testing"}
    )
